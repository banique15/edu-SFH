...
<section class="bg-[#fdf9f1] overflow-hidden">
	...
	<div class="block md:hidden relative w-full min-h-[233px] px-[18px] py-0 max-w-md mx-auto">
		<!-- Title -->
		<h2 class="absolute font-normal text-[32px] leading-none text-black top-0 left-[18px] tracking-[-0.32px] w-[750px] h-[46px]">
			Lessons by Grade
		</h2>
		
		<!-- Grade Selection -->
		<div class="absolute top-[52px] left-[18px] flex items-baseline gap-[32px]">
			<button
				class="relative cursor-pointer bg-transparent border-none p-0"
				onclick={() => handleGradeSelect("5-8")}
			>
				<p class="font-light text-[32px] leading-[0.9] tracking-[-0.32px] text-black"
				   class:opacity-100={selectedGrade === "5-8"}
				   class:opacity-50={selectedGrade !== "5-8"}>
					Grades 5-8
				</p>
				{#if selectedGrade === "5-8"}
					<div class="absolute top-[36px] left-0 w-[136px] h-[3px] bg-[#349552]"></div>
				{/if}
			</button>
			<button
				class="relative cursor-pointer bg-transparent border-none p-0"
		...
		<button
			class="absolute top-[120px] left-[18px] w-[356px] h-[93px] bg-white border-[1.5px] border-black rounded-[20px] flex items-center justify-between px-[28px] cursor-pointer hover:bg-gray-50 transition-colors"
			onclick={handleSeeAllLessons}
		>
			<p class="font-normal text-[32px] leading-none tracking-[-0.32px] text-black w-[224px] h-[45px]">
				See All Lessons
			</p>
			<svg class="w-[56px] h-[29px]" viewBox="0 0 56 29" fill="none">
				<path
					d="M40 14.5L50 14.5M50 14.5L45 9.5M50 14.5L45 19.5"
					stroke="black"
					stroke-width="2"
	...
	<div class="hidden md:block relative h-[182px] mb-[64px] px-6">
