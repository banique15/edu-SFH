<script lang="ts">
	...
	const prompts = $derived(extractQuestions(section.content));
	const encouragement = $derived(extractClosingStatement(section.content));
...
<section class="print:break-inside-avoid">
	...
	<button
		...
		aria-controls="personal-connection-content"
	>
		<div class="flex-1">
			<h2
	...
	<!-- Content -->
	{#if !isCollapsed}
		<div id="personal-connection-content" class="px-0 py-5 print:block">
			{#if prompts.length > 0}
