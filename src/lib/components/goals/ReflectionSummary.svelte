<script lang="ts">
	import { extractOpeningStatement, extractQuestions, replaceLinksWithHtml, convertBoldToHtml } from '$lib/utils/content-parser';
	...
	const openingStatement = $derived(extractOpeningStatement(section.content));
...
<section class="print:break-inside-avoid">
	...
	<!-- Content -->
	{#if !isCollapsed}
		<div id="reflection-content" class="px-0 py-5 print:block">
			{#if openingStatement}
				<div
					class="mb-4 text-base leading-[1.7]"
					style="font-family: 'Founders Grotesk', sans-serif; color: #1F2937;"
				>
					{@html formatText(openingStatement)}
				</div>
			{/if}
