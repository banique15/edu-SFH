<script lang="ts">
	...
	const INITIAL_MATERIALS_COUNT = 6; // 3 per column in 2-column layout
	const visibleMaterials = $derived(
		showAllMaterials ? materials : materials.slice(0, INITIAL_MATERIALS_COUNT)
	);
	const hasMoreMaterials = $derived(materials.length > INITIAL_MATERIALS_COUNT);
	
	// Materials with additional info
	const materialsWithInfo = $derived(
		materials.filter((m) => m.additional_info && m.additional_info.trim() !== '')
	);
				...
				{#if materials.length > 0}
					<div class="flex flex-col gap-2">
						<div class="flex items-center justify-between gap-2">
							<h3
								class="text-sm sm:text-base font-medium leading-[1.2] tracking-[-0.01em] sm:tracking-[-0.16px]"
								style="font-family: 'Founders Grotesk', sans-serif;"
							>
								Materials Needed
							</h3>
							{#if hasMoreMaterials || materialsWithInfo.length > 0}
								<button
									onclick={() => (showAllMaterials = !showAllMaterials)}
									class="text-sm sm:text-base underline hover:opacity-80 transition-opacity flex-shrink-0"
									style="font-family: 'Founders Grotesk', sans-serif;"
								>
									{showAllMaterials 
										? 'Show less' 
										: hasMoreMaterials 
											? `Show ${materials.length - INITIAL_MATERIALS_COUNT} more`
											: 'Show details'}
								</button>
							{/if}
						</div>
						<div class="max-h-[140px] overflow-y-auto">
							<ul class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5">
								{#each visibleMaterials as material (material.id)}
									<li class="flex items-start gap-2">
										...
										<span
											class="text-sm sm:text-base leading-[1.4] tracking-[-0.01em] sm:tracking-[-0.16px]"
											style="font-family: 'Founders Grotesk', sans-serif;"
										>
											{material.name}
							...
							<!-- Additional Info for Materials (only shown when expanded) -->
							{#if showAllMaterials && materialsWithInfo.length > 0}
								<div class="flex flex-col gap-2 mt-3 pt-3 border-t" style="border-color: {sdgTextColor}40;">
									{#each materialsWithInfo as material (material.id)}
										<div class="flex flex-col gap-1">
											<h4
												class="text-sm font-medium leading-[1.2] tracking-[-0.01em]"
												style="font-family: 'Founders Grotesk', sans-serif;"
											>
												{material.name}
											</h4>
											<div
												class="text-xs sm:text-sm leading-[1.4] tracking-[-0.01em] whitespace-pre-line opacity-90"
												style="font-family: 'Founders Grotesk', sans-serif;"
											>
												{material.additional_info}
											</div>
										</div>
									{/each}
								</div>
