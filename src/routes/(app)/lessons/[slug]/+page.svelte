<script lang="ts">
	...
	import MaterialsChecklist from "$lib/components/goals/MaterialsChecklist.svelte";
	...
	const hasLessonComponent = $derived(data.hasLessonComponent);
	const lessonSlug = $derived(data.lessonSlug);

	// Dynamically load the lesson component on the client side
	let lessonComponent = $state<Component | null>(null);

	// Load component when hasLessonComponent is true
	$effect(() => {
		if (hasLessonComponent && lessonSlug && !lessonComponent) {
			loadLessonComponent(lessonSlug, lesson.title).then((component) => {
...
<div class="min-h-screen mt-4 sm:mt-6 p-6">
	...
	{#if materials.length > 0}
		<MaterialsChecklist {materials} {sdgColor} lessonId={lesson.id} />
	{/if}
	...
	<div class="max-w-7xl mx-auto py-6 sm:py-8">
		<div class="space-y-6">
			{#if lessonComponent}
				<!-- Render Svelte lesson component -->
				<svelte:component this={lessonComponent} {sdgColor} />
			{:else if hasLessonComponent}
				<!-- Component is loading -->
				<div class="text-center py-8">
					<div class="text-gray-500">Loading lesson content...</div>
				</div>
			{:else}
				<!-- Fallback to old content sections -->
				<!-- Content Sections -->
				{#each content as section (section.id)}
				<section class="print:break-inside-avoid">
					<!-- Section Header -->
					<button
						class="w-full text-left px-0 py-4 flex items-center justify-between border-b border-gray-300 transition-colors print:cursor-default"
						onclick={() => toggleSection(`content-${section.id}`)}
					>
						<div class="flex-1">
							<h2
						...
						<svg
							...
							class:rotate-180={collapsedSections[
								`content-${section.id}`
							]}
							fill="none"
							stroke="currentColor"
					...
					<!-- Section Content -->
					{#if !collapsedSections[`content-${section.id}`]}
						<div class="px-0 py-5">
							<div
