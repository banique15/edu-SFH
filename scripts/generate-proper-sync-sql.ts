...
// [All the helper functions from before - parseFrontmatter, normalizeArtForm, parseDuration, escapeSql]
...
async function generateSQL() {
	...
	// Generate SQL that uses ROW_NUMBER() to assign lesson numbers starting from max+1
	let sql = `-- Insert missing lessons with proper lesson_number assignment
...
WHERE NOT EXISTS (
  SELECT 1 FROM edu_lessons WHERE edu_lessons.title = new_lessons.title
	...
	writeFileSync(join(__dirname, '../sync-lessons-final.sql'), sql, 'utf-8');
	console.log('✅ Generated sync-lessons-final.sql');
